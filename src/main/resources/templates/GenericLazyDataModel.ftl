...
abstract class GenericLazyDataModel<T> extends LazyDataModel<T> implements Serializable {
    ...
    protected abstract String getStrId(T item);
    ...
    public String getRowKey(T object) {
        return getStrId(object);
    }

    @Override
    public T getRowData(String rowKey) {
        for (T customer : datasource) {
            if (getStrId(customer).equals(rowKey)) {
                return customer;
            }
        }
        return null;
